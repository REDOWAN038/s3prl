    ...
    def forward(self, features, lens, targets = None):
        ...
        if self.ar:
            ...
            for t, encoder_state in enumerate(encoder_states.transpose(0, 1)):
            	...
            	projected = self.proj(z_list[-1]).view(B, self.output_dim, -1)
            	predicted_list.append(projected)

            	print("taget type ", type(targets))

            	if targets is not None and t < targets.size(0):
            		prev_out = targets[t]
            	else:
            		prev_out = projected.squeeze(-1)
